/*
  ...
  private[security] class HiveCredentialProvider extends ServiceCredentialProvider with Logging {
    ...
    override def obtainCredentials(
      ...
      logInfo(s"hive.sentry.conf.url:${sentryurl.toString}")
        ...
        doAsRealUser {
          val hive = {
            if (sparkConf.get("spark.sentry.enabled", "false").toBoolean) {
              ...
              val username = if ("cluster".equals( sparkConf.get("spark.submit.deployMode" ))) {
                sparkConf.get("hive.sentry.subject.name")
              } else {
                System.getProperty( "hive.sentry.subject.name" )
              }
              ...
              hadoopConf.set("hive.sentry.subject.name", username)
              logInfo(s"obtainCredentials doAsRealUser : ${url}")
              val conf = hiveConf(hadoopConf)
